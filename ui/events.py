...
class EventHandler:
    ...
    def on_mouse_move(self, event):
        self.canvas_drawer.last_mouse_pos = (event.x, event.y)
        ...
        for x_val in self.stations:
            for _, y, z in self.stations[x_val]:
                px, py = self.canvas_drawer.project_point(x_val, y, z)

                # Detect mouse near point
                if abs(px - x_mouse) < 5 and abs(py - y_mouse) < 5:
                    # Update tooltip
                    self.tooltip.config(text=f"Y={y:.3f}, Z={z:.3f}")

                    # delete tooltip
                    if self.tooltip_window:
                        self.canvas.delete(self.tooltip_window)

                    # create new tooltip
                    self.tooltip_window = self.canvas.create_window(
                        event.x + 10, event.y + 10,
                        window=self.tooltip, anchor="nw"
                    )
                    found = True
                    break
        ...
        if not found and self.tooltip_window:
            self.canvas.delete(self.tooltip_window)
    ...
    # --- Project Isometric ---
    def canvas_to_iso_coords(self, canvas_x, canvas_y):
        cx, cy = self.offsets['iso']
        scale = self.iso_scale
        pan_x, pan_y = self.view_pan_iso
        iso_x = (canvas_x - cx - pan_x)/scale
        iso_y = (cy - canvas_y + pan_y)/scale
        return iso_x, iso_y
    ...
    def change_angle(self):
        ...
        sx = self.right_click_station
        idx = self.right_click_point_index
        if sx is None or idx is None:
            return

        angle_deg = sd.askfloat("Change Angle", "Input Angle in degree:")
        ...
        self.stations[sx][idx]["angle"] = angle_deg
        self.stations[sx][idx]["tangent"] = [0, tx, tz]  
